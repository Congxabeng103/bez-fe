...
export function Profile() {
  ...
  // 1. Thêm State lưu lỗi cho Profile (Validate Họ Tên)
  const [profileErrors, setProfileErrors] = useState({ firstName: "", lastName: "" });

  // --- State Password & Address ---
  ...
  // (Giữ nguyên phần useEffect API Địa chỉ...)
  useEffect(() => {
  ...
  // --- 1. XỬ LÝ CẬP NHẬT PROFILE (VALIDATE TÊN) ---
  const handleUpdateProfile = async () => {
    let hasError = false;
    const newErrors = { firstName: "", lastName: "" };

    // Regex: Chỉ chấp nhận chữ cái (Unicode cho tiếng Việt) và khoảng trắng. KHÔNG số, KHÔNG ký tự đặc biệt
    const nameRegex = /^[\p{L}\s]+$/u; 

    if (!formData.firstName.trim()) {
        newErrors.firstName = "Tên là bắt buộc"; hasError = true;
    } else if (!nameRegex.test(formData.firstName.trim())) {
        newErrors.firstName = "Tên không được chứa số hoặc ký tự đặc biệt"; hasError = true;
    }

    if (!formData.lastName.trim()) {
        newErrors.lastName = "Họ là bắt buộc"; hasError = true;
    } else if (!nameRegex.test(formData.lastName.trim())) {
        newErrors.lastName = "Họ không được chứa số hoặc ký tự đặc biệt"; hasError = true;
    }

    setProfileErrors(newErrors);
    if (hasError) return; // Dừng nếu lỗi
  ...
  // --- 2. XỬ LÝ ĐỔI MẬT KHẨU (CẬP NHẬT REGEX KÝ TỰ ĐẶC BIỆT) ---
  const handleChangePassword = async () => {
    ...
    // --- Regex Mới: Bắt buộc có ký tự KHÔNG phải chữ/số (tức là ký tự đặc biệt) ---
    // Min 8 chars, 1 Lower, 1 Upper, 1 Number, 1 Special Char
    const passwordRegex = /^(?=.*[a-z])(?=.*[A-Z])(?=.*\d)(?=.*[^a-zA-Z0-9]).{8,}$/;
    
    if (!passwordRegex.test(passwordData.newPassword)) {
        newErrors.newPassword = "Mật khẩu cần: 8 ký tự, chữ hoa, chữ thường, số và ký tự đặc biệt (!@#_-%...)";
        hasError = true;
    }
        ...
        <Card className="lg:col-span-2">
          <CardHeader>
             <div className="flex gap-2 flex-wrap">
              <Button variant={mode === "profile" ? "default" : "outline"} onClick={() => setMode("profile")} className="flex-1 min-w-[150px]">Thông tin cá nhân</Button>
              <Button variant={mode === "address" ? "default" : "outline"} onClick={() => setMode("address")} className="flex-1 min-w-[150px]">Địa chỉ mặc định</Button>
              <Button variant={mode === "password" ? "default" : "outline"} onClick={() => setMode("password")} className="flex-1 min-w-[150px]">Đổi mật khẩu</Button>
            </div>
          </CardHeader>
          <CardContent className="space-y-4">
            ...
            {mode === "profile" && (
              <form onSubmit={(e) => { e.preventDefault(); handleUpdateProfile(); }} className="space-y-4">
                <div className="space-y-1.5">
                    ...
                    <ImageUpload value={formData.avatar} onChange={(url) => setFormData(prev => ({ ...prev, avatar: url }))} label="" className="w-24 h-24" />
                </div>
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                  {/* Validate Họ */}
                  <div>
                    <Label htmlFor="lastName" className={profileErrors.lastName ? "text-red-500" : ""}>Họ (và tên đệm) *</Label>
                    <Input 
                        id="lastName" name="lastName" 
                        value={formData.lastName} onChange={handleFormChange} 
                        placeholder="Vd: Đỗ Thành" 
                        className={`mt-1.5 ${profileErrors.lastName ? "border-red-500 focus-visible:ring-red-500" : ""}`}
                    />
                  ...
                  {/* Validate Tên */}
                  <div>
                    <Label htmlFor="firstName" className={profileErrors.firstName ? "text-red-500" : ""}>Tên *</Label>
                    <Input 
                        id="firstName" name="firstName" 
                        value={formData.firstName} onChange={handleFormChange} 
                        placeholder="Vd: Công" 
                        className={`mt-1.5 ${profileErrors.firstName ? "border-red-500 focus-visible:ring-red-500" : ""}`}
                    />
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                  <div>
                    ...
                    <Select name="gender" value={formData.gender || "OTHER"} onValueChange={(value) => setFormData(prev => ({ ...prev, gender: value }))}>
                      <SelectTrigger id="gender" className="mt-1.5"><SelectValue placeholder="Chọn giới tính" /></SelectTrigger>
                      <SelectContent>
            ...
            {/* TAB 2: ADDRESS (NO VALIDATE) */}
            {mode === "address" && (
              <form onSubmit={(e) => { e.preventDefault(); handleUpdateAddress(); }} className="space-y-4">
                 <div className="space-y-1.5">
                   <Label htmlFor="province">Tỉnh/Thành phố</Label>
                   <Select value={selectedProvince?.code.toString() || ""} onValueChange={(value) => { const p = provinces.find(i => i.code === parseInt(value)); setSelectedProvince(p || null); }} disabled={isLoadingProvinces}>
                     <SelectTrigger id="province"><SelectValue placeholder={isLoadingProvinces ? "Đang tải..." : "Chọn Tỉnh/Thành"} /></SelectTrigger>
                     <SelectContent>{provinces.map(p => (<SelectItem key={p.code} value={p.code.toString()}>{p.name}</SelectItem>))}</SelectContent>
                   </Select>
                 </div>
                 <div className="space-y-1.5">
                   <Label htmlFor="district">Quận/Huyện</Label>
                   <Select value={selectedDistrict?.code.toString() || ""} onValueChange={(value) => { const d = districts.find(i => i.code === parseInt(value)); setSelectedDistrict(d || null); }} disabled={!selectedProvince || isLoadingDistricts}>
                     <SelectTrigger id="district"><SelectValue placeholder={isLoadingDistricts ? "Đang tải..." : "Chọn Quận/Huyện"} /></SelectTrigger>
                     <SelectContent>{districts.map(d => (<SelectItem key={d.code} value={d.code.toString()}>{d.name}</SelectItem>))}</SelectContent>
                   </Select>
                 </div>
                 <div className="space-y-1.5">
                   <Label htmlFor="ward">Phường/Xã</Label>
                   <Select value={selectedWard?.code.toString() || ""} onValueChange={(value) => { const w = wards.find(i => i.code === parseInt(value)); setSelectedWard(w || null); }} disabled={!selectedDistrict || isLoadingWards}>
                     <SelectTrigger id="ward"><SelectValue placeholder={isLoadingWards ? "Đang tải..." : "Chọn Phường/Xã"} /></SelectTrigger>
                 ...
                 <div className="space-y-1.5">
                   ...
                   <Input id="streetAddress" name="streetAddress" placeholder="Ví dụ: 123 Đường Nguyễn Huệ" value={streetAddress} onChange={(e) => setStreetAddress(e.target.value)} />
                 </div>
                 <Button type="submit" className="w-full" disabled={loading}>
                   {loading && <Loader2 size={16} className="animate-spin mr-2" />}
                   Lưu địa chỉ
            ...
            {/* TAB 3: PASSWORD (VALIDATE INLINE + SPECIAL CHAR) */}
            {mode === "password" && (
              <form onSubmit={(e) => { e.preventDefault(); handleChangePassword(); }} className="space-y-4">
                <div>
                   <Label htmlFor="currentPassword" className={passwordErrors.currentPassword ? "text-red-500" : ""}>Mật khẩu cũ *</Label>
                   <Input 
                      ...
                      className={`mt-1.5 ${passwordErrors.currentPassword ? "border-red-500 focus-visible:ring-red-500" : ""}`}
                    />
                    {passwordErrors.currentPassword && <p className="text-xs text-red-500 mt-1">{passwordErrors.currentPassword}</p>}
                 </div>
                 <div>
                   <Label htmlFor="newPassword" className={passwordErrors.newPassword ? "text-red-500" : ""}>Mật khẩu mới *</Label>
                   <Input 
                      id="newPassword" name="newPassword" type="password" 
                      value={passwordData.newPassword} onChange={handlePasswordChange} 
                      placeholder="Nhập mật khẩu mới" 
                      className={`mt-1.5 ${passwordErrors.newPassword ? "border-red-500 focus-visible:ring-red-500" : ""}`}
                    />
                 ...
                 <div>
                   <Label htmlFor="confirmationPassword" className={passwordErrors.confirmationPassword ? "text-red-500" : ""}>Xác nhận mật khẩu mới *</Label>
                   <Input 
                      id="confirmationPassword" name="confirmationPassword" type="password" 
                      value={passwordData.confirmationPassword} onChange={handlePasswordChange} 
                      placeholder="Xác nhận mật khẩu mới" 
                      className={`mt-1.5 ${passwordErrors.confirmationPassword ? "border-red-500 focus-visible:ring-red-500" : ""}`}
                    />
