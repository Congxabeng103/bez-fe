...
// --- Interfaces (Cập nhật để khớp với logic mới) ---
...
// Dùng để lấy chi tiết SP và các options của nó
interface ProductDetailResponse {
  product: ProductBrief; // (Chỉ cần thông tin cơ bản)
  attributes: ProductOptionResponse[]; // Đây là các options
}
...
export function VariantManagement() {
  ...
  // State mới: Chứa các options của riêng sản phẩm đang chọn
  ...
  const fetchProductDetailsAndOptions = useCallback(async (productId: string) => {
    ...
    try {
      const response = await fetch(`${API_URL}/v1/products/detail/${productId}`, { 
        headers: { "Authorization": `Bearer ${token}` } 
      });
      if (!response.ok) { throw new Error("Lỗi tải thuộc tính của sản phẩm"); }
      const result = await response.json();
      if (result.status === 'SUCCESS' && result.data) {
  ...
  const handleSaveVariants = async () => {
    ...
    // 2. Map dữ liệu thô (raw data) từ state, bao gồm cả data tạm
    const variantsRawData = generatedCombinations.map(combo => {
      const originalAttributes = combo.attributes; // { "Màu sắc": "Đỏ", "Kích cỡ": "S" }
      ...
      return {
        ...
        // Dữ liệu tạm để kiểm tra
        _originalAttributeCount: Object.keys(originalAttributes).length 
      };
    });

    // 3. Thực hiện kiểm tra trên dữ liệu thô
    // So sánh: số ID tìm được (v.optionValueIds.length) 
    // với: số thuộc tính gốc (v._originalAttributeCount)
    if (variantsRawData.some(v => v.optionValueIds.length !== v._originalAttributeCount)) {
        ...
        console.error("Failed to find option value IDs for some combinations:", variantsRawData);
        return;
    }

    // 4. Tạo request body SẠCH, chỉ chứa những gì API cần
    const batchRequest = {
      ...
      variants: variantsRawData.map(v => ({
        ...
        // (Trường _originalAttributeCount đã bị loại bỏ)
      }))
    };
    ...
    try {
      const response = await fetch(`${API_URL}/v1/variants/batch`, {
        ...
        body: JSON.stringify(batchRequest) // Gửi request body SẠCH
      });
      const result = await response.json();
  ...
  const generateCombinations = () => {
    // Chỉ chọn từ các options mà sản phẩm này có
  ...
  const handlePermanentDeleteVariant = async (id: number) => {
    ...
    try {
      const response = await fetch(`${API_URL}/v1/variants/${id}/permanent`, { method: "DELETE", headers: { "Authorization": `Bearer ${token}` } });
      if (!response.ok) {
