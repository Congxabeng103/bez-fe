...
export function middleware(request: NextRequest) {
  ...
  try {
    ...
    // Kiểm tra các role nhân viên/quản trị
    const isAdminOrStaffOrManager = 
      userRoles.includes('ADMIN') || 
      userRoles.includes('STAFF') || 
      userRoles.includes('ROLE_STAFF') || // Thêm
      userRoles.includes('MANAGER') || 
      userRoles.includes('ROLE_MANAGER'); // Thêm

    if (isAdminOrStaffOrManager) {
      // ADMIN/STAFF/MANAGER vào trang admin (vd: /admin/products) -> Cho phép
      if (pathname.startsWith(adminPath)) {
        return NextResponse.next();
      }
      // ADMIN/STAFF/MANAGER vào các trang public khác (/, /products) -> Cũng cho phép
      // (Quan trọng: Nếu họ ở trang /login, chúng ta nên đá họ về /admin)
      if (pathname === '/login' || pathname === '/register') {
        console.log('Middleware: Admin/Staff/Manager on auth page -> redirecting to admin home');
    ...
    // Nếu là USER (hoặc role chỉ có USER)
    if (userRoles.includes('USER') || userRoles.includes('ROLE_USER')) {
      ...
      // (Quan trọng: Nếu họ ở trang /login, chúng ta nên đá họ về /)
      if (pathname === '/login' || pathname === '/register') {
        console.log('Middleware: USER on auth page -> redirecting to user home');
        return NextResponse.redirect(new URL('/', request.url));
      }
    ...
    // (Role không xác định, hoặc không có role nào)
    console.log('Middleware: Unknown or empty roles -> redirecting to login');
    const response = NextResponse.redirect(new URL('/login', request.url));
    response.cookies.delete('authToken'); // Xóa cookie vì
    return response;
...
export const config = {
  matcher: [
     ...
     */
    // Sửa: Bỏ các trang auth ra khỏi (?!...)
    // Middleware sẽ chạy trên MỌI TRANG
    // trừ api, _next/static, _next/image, và favicon.ico
